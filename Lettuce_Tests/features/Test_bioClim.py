...
@step(r'Given a bunch of inputs for prism')
def bioclim_constants(step):
    ...
    world.prcp_var = "ppt"
    world.tave_var = "NULL"
    world.output="/Users/dblodgett/temp/test_bioclim.zip"
    

@step(r'When I execute the bioclim algorithm')
def run_bioclim(step):
    bioclim.submitCustomBioclim(outputfname=world.output, 
...
@step(r'Then I want to make sure the file is what I expect from before')
def check_file_size(step):
    assert_equal(world.output, 'http://bogus/bogy/geoserver/wfs')
